...
/**
 * BTC 15m Up/Down history saver + live appender
 *
 * Features:
 * - Backfill historical resolved 15m BTC markets
 * - Append each row to CSV immediately
 * - Keep watching for newly closed markets in near real time
 *
 * Usage:
 *   node 15min_result_live.js --days=3
 *   node 15min_result_live.js --days=1 --out=btc_15m_results_live.csv
 *   node 15min_result_live.js --start=2026-03-20T00:00:00Z --end=2026-03-24T23:59:59Z
 *
 * Node.js 18+
 */
...
const DEFAULT_OUT = "btc_15m_results_live.csv";
const HEADER = [
  "slug",
  "market_id",
  "question",
...
function floorTo15Min(tsSec) {
  return Math.floor(tsSec / 900) * 900; // Adjusted for 15-minute intervals
}
...
function buildRow(slug, market, winner) {
  const ts = Number((slug.match(/^btc-updown-15m-(\d{10})$/) || [])[1] || NaN); // Updated regex for 15m
  const endTs = Number.isFinite(ts) ? ts + 900 : null; // Adjusted for 15-minute interval
  const outcomes = safeJsonParse(market?.outcomes, market?.outcomes);
  const outcomePrices = safeJsonParse(market?.outcomePrices, market?.outcomePrices);
...
function getBackfillRange(opts) {
  if (opts.start || opts.end) {
    ...
    return {
      startSec: floorTo15Min(Math.floor(startMs / 1000)),
      endSec: floorTo15Min(Math.floor(endMs / 1000)),
    };
  }

  const nowSec = Math.floor(Date.now() / 1000);
  const lastClosedMarketStartSec = floorTo15Min(nowSec) - 900; // Adjusted for 15-minute interval
  const intervals = Math.max(1, Math.floor((opts.days * 24 * 60 * 60) / 900)); // Adjusted for 15-minute interval

  return {
    startSec: lastClosedMarketStartSec - (intervals - 1) * 900, // Adjusted for 15-minute interval
...
async function backfill(opts, outPath, savedSlugs) {
  ...
  for (let ts = startSec; ts <= endSec; ts += 900) { // Adjusted for 15-minute interval
...
function getLastClosedMarketStartSec() {
  const nowSec = Math.floor(Date.now() / 1000);
  return floorTo15Min(nowSec) - 900; // Adjusted for 15-minute interval
}
...
async function watchLive(opts, outPath, savedSlugs) {
  ...
  while (true) {
    try {
      ...
      // only one newly closed market every 15 minutes, but this catches missed ones too
      for (let ts = lastClosedStartSec - 3600; ts <= lastClosedStartSec; ts += 900) { // Adjusted for 15-minute interval
        const slug = `btc-updown-15m-${ts}`;
